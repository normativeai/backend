      ...
      theory.isConsistent(function(code, cons) {
        if (code == 1) { // mleancop ok
          if (cons) {
            ...
            res.status(206).json({message: 'Backend prover could not resolve the consistency of this legislation wthin 5 seconds. This normally means (but not always) that the legilsation is consistent!', type: 'warning'});
        } else {
      ...
      theory.isIndependent(req.params.formId, function(code, cons) {
        if (code == 1) { // mleancop ok
          if (cons) {
            ...
            res.status(206).json({message: 'Backend prover could not resolve the independency of this norm within 5 seconds. This normally means (but not always) that the norm is independent!', type: 'warning'});
        } else { //mleancop error
