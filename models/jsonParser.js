  ...
  switch (obj.connective.code) {
    case "label":
      ...
      if (state.pass == 1) {
        ...
        state.map.set(label, formula)
      } else if (state.pass == 3) {
        return state.map.get(label)
      }
    case "exception":
      /*
...
function parse(obj, state) {
  // if state, then we do multiple passes, otherwise, we just extract formulae and report an error if it contains steps
  // for passes 1 and 2
  if (state) {
    parseFormula(obj, {pass: 1, map: state})
    parseFormula(obj, {pass: 2, map: state})
    return parseFormula(obj, {pass: 3, map: state})
  } else {
    return parseFormula(obj)
  }
}

module.exports  = { "parseFormula": parse, "extractViolations": extractViolations,"arities": expectedArgs };
