...
function identBinary(w1,f1,w2,f2,state,w3) {
  ...
  return `${ident(state)}${w1}<BR/>${ident(inc(state))}${f1}<BR/>${ident(state)}${w2}<BR/>${ident(inc(state))}${f2}${w3s}<BR/>`;
}

function identUnary(w1,f1,state) {
  return `${ident(state)}${w1}<BR/>${ident(inc(state))}${f1}`;
}

function identMulti(w,fs,state) {
  return fs.slice(1).reduce(function(acc, val) {
    return `${acc}<BR/>${ident(state)}${w} ${val}`
  }, `${ident(state)}${fs[0]}`);
}
